...
green "例：原二级域名 x.zz.eu.org 或 x.zz.cf ，在cloudflare中重命名其中的x名称，确定并生效"
echo
yellow "建议二：更换下当前本地网络IP环境，再尝试执行脚本" && exit
}
if [[ -f /root/zz/cert.crt && -f /root/zz/private.key ]] && [[ -s /root/zz/cert.crt && -s /root/zz/private.key ]]; then
sed -i '/--cron/d' /etc/crontab
echo "0 0 * * * root bash ~/.acme.sh/acme.sh --cron -f >/dev/null 2>&1" >> /etc/crontab
green "域名证书申请成功或已存在！域名证书（cert.crt）和密钥（private.key）已保存到 /root/zz文件夹内" 
yellow "公钥文件crt路径如下，可直接复制"
green "/root/zz/cert.crt"
yellow "密钥文件key路径如下，可直接复制"
green "/root/zz/private.key"
echo $ym > /root/zz/ca.log
...
installCA(){
bash ~/.acme.sh/acme.sh --install-cert -d ${ym} --key-file /root/zz/private.key --fullchain-file /root/zz/cert.crt --ecc
}
...
echo -e "${bblue}     ░██              ░██ ${plain} "
echo -e "${bblue}    ░██              ░██${plain} "
echo -e "${bblue}   ░██${plain}              ░██${plain}  "
echo -e "${bblue} ░████████${plain}       ░████████${plain}  "
green "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" 
yellow "感谢x-ui代码贡献者们（neil）"
yellow "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" 
yellow " 提示："
...
yellow " 公钥文件crt保存路径：/root/zz/cert.crt"
yellow " 密钥文件key保存路径：/root/zz/private.key"
echo
red "========================================================================="
acmeshow
